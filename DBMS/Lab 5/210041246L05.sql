...
--1--
SELECT ACT_FIRSTNAME, COUNT(*) AS TOTAL_ACTRESSES
FROM ACTOR
WHERE ACT_GENDER = 'F'
...
--3--
SELECT TO_CHAR(MOV_RELEASEDATE, 'MM') AS RELEASE_MONTH,
       TO_CHAR(MOV_RELEASEDATE, 'Month') AS MONTH_NAME,
       COUNT(*) AS MOVIE_COUNT
FROM MOVIE
GROUP BY TO_CHAR(MOV_RELEASEDATE, 'MM'), TO_CHAR(MOV_RELEASEDATE, 'Month')
ORDER BY RELEASE_MONTH;

--4--
SELECT
    TO_CHAR(MIN(MOVIE.MOV_RELEASEDATE), 'Month YYYY') AS FIRST_MOVIE_RELEASE_MONTH,
    TO_CHAR(MAX(MOVIE.MOV_RELEASEDATE), 'Month YYYY') AS LAST_MOVIE_RELEASE_MONTH
FROM MOVIE
WHERE MOVIE.MOV_ID IN (
    SELECT DIRECTION.MOV_ID
    FROM DIRECTION
    WHERE DIRECTION.DIR_ID = (
...
--5--
SELECT REV_ID, COUNT(*) AS LOWEST_REV_STAR_COUNT
FROM (
    SELECT R.REV_ID, MIN(R.REV_STARS) AS MIN_REV_STAR
    FROM RATING R
    GROUP BY R.REV_ID
) LowestRatings
GROUP BY REV_ID
HAVING COUNT(*) = (
    SELECT MAX(LOWEST_REV_STAR_COUNT)
    FROM (
        SELECT REV_ID, COUNT(*) AS LOWEST_REV_STAR_COUNT
        FROM (
            SELECT R.REV_ID, MIN(R.REV_STARS) AS MIN_REV_STAR
...
--6--
SELECT MOVIE.MOV_TITLE, COALESCE(AVG(RATING.REV_STARS), 0) AS AVERAGE_RATING
FROM MOVIE
LEFT JOIN RATING ON MOVIE.MOV_ID = RATING.MOV_ID
...
SELECT MOV_ID,
       CASE
           WHEN MOV_YEAR < 1980 THEN MOV_TITLE || ' (old gold)'
           WHEN MOV_YEAR >= 1980 AND MOV_YEAR <= 2000 THEN MOV_TITLE || ' (trendy 90''s)'
           WHEN MOV_YEAR > 2000 THEN MOV_TITLE || ' (weird 20''s)'
           ELSE MOV_TITLE
       END AS MOV_TITLE_WITH_SUFFIX
FROM MOVIE;

--8--
SELECT M.MOV_TITLE, A.ACT_FIRSTNAME AS ACTOR_FIRSTNAME, A.ACT_LASTNAME AS ACTOR_LASTNAME,
       D.DIR_FIRSTNAME AS DIRECTOR_FIRSTNAME, D.DIR_LASTNAME AS DIRECTOR_LASTNAME
FROM MOVIE M
...
AS SELECT MOV_ID, REV_ID, REV_STARS FROM RATING;
