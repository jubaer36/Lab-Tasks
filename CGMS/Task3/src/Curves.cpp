...
const blaze::StaticMatrix<double, 4UL, 4UL> B_Hermite =
{
	{1.0,  0.0, -3.0,  2.0},
	{0.0,  0.0,  3.0, -2.0},
	{0.0,  1.0, -2.0,  1.0},
...
Image& DrawLineLow(Image& image, const Point& p0, const Point& p1, const Color& color)
{
	...
	for (int x = p0.X; x <= p1.X; x++)
	{
		if (x >= 0 && x < image.GetWidth() &&
			y >= 0 && y < image.GetHeight()) image.Pixel(y, x) = color;
...
Image& DrawHermiteCurve(Image& image, const std::vector<Point>& points, const Color& color, double samplingInterval)
{
	...
	{
		auto P = G * B_Hermite * Tfunc(t);
		if (P[0] >= 0 && P[0] <= image.GetWidth() &&
			P[1] >= 0 && P[1] <= image.GetHeight())
		{
...
Image& DrawCurve(Image& image, CurveType curveType, const std::vector<Point>& points,
	int circleRadius, const Color& curveColor, const Color& lineColor,
	const Color& tangentColor, const Color& circleColor, double samplingInterval)
{
	if (curveType == CurveType::HERMITE)
	{
		DrawCircle(image, points[0], circleRadius, circleColor);
		DrawCircle(image, points[1], circleRadius, circleColor);
		DrawLine(image, points[0],
			{ points[0].X + points[2].X / TANGENT_VISLEN_DIVISOR, points[0].Y + points[2].Y / TANGENT_VISLEN_DIVISOR },
			tangentColor);
		DrawLine(image, points[1],
			{ points[1].X - points[3].X / TANGENT_VISLEN_DIVISOR, points[1].Y - points[3].Y / TANGENT_VISLEN_DIVISOR },
			tangentColor);
	}
	else
	{
	...
	case CurveType::HERMITE:
		DrawHermiteCurve(image, points, curveColor, samplingInterval);
		break;
	...
	case CurveType::BSPLINE:
		if (points.size() == 4)
			DrawBSplineSegment(image, points, curveColor, samplingInterval);
		else
