...
void pmem_init(unsigned int mbi_addr)
{
    ...
    {
        at_set_perm(i, 1); // Reserved by kernel, permission 1.
    }
    ...
    for (unsigned int i = VM_USERLO_PI; i < VM_USERHI_PI; i++)
    {
        ...
        {
            unsigned int start = get_mms(j) / PAGESIZE;
            unsigned int length = get_mml(j) / PAGESIZE;
            unsigned int end = start + length;

            if (is_usable(j) == 1 && i >= start && i < end)
            {
                usable = 1;
                break;
            }
