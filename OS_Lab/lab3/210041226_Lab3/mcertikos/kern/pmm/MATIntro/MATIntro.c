...
unsigned int NUM_PAGES;  // Removed 'static' to allow global access
 ...
 */
static struct ATStruct AT[1 << 20];  // Static is fine here, since this is internal

// The getter function for NUM_PAGES.
unsigned int get_nps(void)  // Removed gcc_inline
{
    return NUM_PAGES;
}

// The setter function for NUM_PAGES.
void set_nps(unsigned int nps)  // Removed gcc_inline
{
    NUM_PAGES = nps;
}
